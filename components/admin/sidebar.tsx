    ...
    <aside className="hidden border-r border-border bg-background md:flex md:w-64 md:shrink-0 md:flex-col">
      ...
      <div className="border-t border-border p-4">
        <Link
          ...
          className="mb-2 flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-foreground/70 hover:bg-muted hover:text-foreground"
        >
          <ExternalLink className="h-4 w-4" />
          Náhled webu
        </Link>
        {userEmail ? (
          <p className="mb-2 truncate px-3 text-xs text-muted-foreground" title={userEmail}>
            {userEmail}
          </p>
        ) : null}
